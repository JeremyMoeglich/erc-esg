...
// .svelte-kit/output/server/entries/endpoints/api/get_articles/_server.ts.js
  ...
  ".svelte-kit/output/server/entries/endpoints/api/get_articles/_server.ts.js"() {
    ...
    POST = async ({ request }) => {
      const { start, end, filter } = await get_request_body(
        request,
        mod.object({
          start: mod.number().int().nonnegative(),
          end: mod.number().int().nonnegative(),
...
// .svelte-kit/vercel-tmp/api/get_articles/manifest.js
var manifest = {
  appDir: "_app",
  appPath: "_app",
...
// .svelte-kit/vercel-tmp/api/get_articles/edge.js
