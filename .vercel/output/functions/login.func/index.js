...
// .svelte-kit/output/server/chunks/input_field.js
function idify(obj) {
  obj = obj.toLowerCase();
  obj = obj.replace(/[^a-z0-9]/g, "_");
  return obj.replace(/_+/g, "_");
}
var css3, Input_field;
var init_input_field = __esm({
  ".svelte-kit/output/server/chunks/input_field.js"() {
    init_chunks();
    css3 = {
      code: "div.svelte-58rmm0{display:flex;gap:5px;width:100%;padding:10px 30px;flex-direction:column}label.svelte-58rmm0{font-weight:bold;color:var(--gray800)}textarea.svelte-58rmm0{resize:vertical}input.svelte-58rmm0,textarea.svelte-58rmm0{border:1px solid var(--gray800);border-radius:3px;padding:5px;font-size:large;color:var(--gray800);background-color:var(--gray100);width:100%}input.svelte-58rmm0:focus,textarea.svelte-58rmm0:focus{padding:4px;outline:none;border:2px solid var(--secondary-color);color:var(--secondary-color)}",
      map: null
    };
    Input_field = create_ssr_component(($$result, $$props, $$bindings, slots) => {
      let { type = "text" } = $$props;
      let { value = "" } = $$props;
      ...
      if ($$props.text_field === void 0 && $$bindings.text_field && text_field !== void 0)
        $$bindings.text_field(text_field);
      $$result.css.add(css3);
      return `<div class="${"svelte-58rmm0"}"><label${add_attribute("for", idify(text), 0)} class="${"svelte-58rmm0"}">${escape2(text)}</label>
	${text_field ? `<textarea${add_attribute("id", idify(text), 0)}${add_attribute("placeholder", placeholder, 0)}${add_attribute("autocomplete", autocomplete, 0)} ${required ? "required" : ""}${add_attribute("name", idify(text), 0)}${add_attribute("title", placeholder, 0)} class="${"svelte-58rmm0"}">${value || ""}</textarea>` : `<input${add_attribute("type", type, 0)}${add_attribute("value", value, 0)}${add_attribute("autocomplete", autocomplete, 0)}${add_attribute("name", idify(text), 0)}${add_attribute("title", placeholder, 0)} ${required ? "required" : ""}${add_attribute("placeholder", placeholder, 0)} class="${"svelte-58rmm0"}"${add_attribute("this", input_element, 0)}>`}
</div>`;
    });
  }
});
...
async function login(email, password) {
  const { token } = mod.object({
    token: mod.string()
  }).parse(
    await (await fetch("/auth/login", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
...
var init_page_svelte = __esm({
  ".svelte-kit/output/server/entries/pages/login/_page.svelte.js"() {
    ...
    init_loading_store();
    init_functional_utilities();
    UserAvatar = create_ssr_component(($$result, $$props, $$bindings, slots) => {
      let labelled;
      let attributes;
      ...
      return `<svg${spread(
        ...
        {}
      )}>${title ? `<title>${escape2(title)}</title>` : ``}<path d="${"M16,8a5,5,0,1,0,5,5A5,5,0,0,0,16,8Zm0,8a3,3,0,1,1,3-3A3.0034,3.0034,0,0,1,16,16Z"}"></path><path d="${"M16,2A14,14,0,1,0,30,16,14.0158,14.0158,0,0,0,16,2ZM10,26.3765V25a3.0033,3.0033,0,0,1,3-3h6a3.0033,3.0033,0,0,1,3,3v1.3765a11.8989,11.8989,0,0,1-12,0Zm13.9925-1.4507A5.0016,5.0016,0,0,0,19,20H13a5.0016,5.0016,0,0,0-4.9925,4.9258,12,12,0,1,1,15.985,0Z"}"></path></svg>`;
    });
    css4 = {
      code: ".outer.svelte-1ewt7kf{display:flex;justify-content:center;width:100%}.btn.svelte-1ewt7kf{align-items:center}.inputs.svelte-1ewt7kf{display:flex;gap:var(--distance);width:100%;flex-direction:column;align-items:center}.avatar.svelte-1ewt7kf{display:flex;width:90%;justify-content:center;padding-bottom:20px;border-bottom:2px solid var(--gray500)}.register.svelte-1ewt7kf{display:flex;flex-direction:column;align-items:center;gap:10px;width:90%;border-top:2px solid var(--gray500);padding-top:20px;margin-top:40px}.main.svelte-1ewt7kf{display:flex;align-items:center;flex-direction:column;padding:30px;margin:50px;background-color:var(--gray100);min-height:700px;width:min(100%, 600px);box-shadow:0px 5px 10px rgba(0, 0, 0, 0.205);border-radius:10px}",
      map: null
    };
    Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
      let $current_auth_state, $$unsubscribe_current_auth_state;
      let $$unsubscribe_page;
      $$unsubscribe_current_auth_state = subscribe(current_auth_state, (value) => $current_auth_state = value);
      ...
      async function run_page_mode() {
        if (page_mode === "register" && field_values["Passwort"] !== field_values["Passwort wiederholen"]) {
          error_message = "Passw\xF6rter stimmen nicht \xFCberein.";
          return;
        }
        try {
          is_loading.set(true);
          const response = page_mode === "login" ? await login(field_values.Email, field_values.Passwort) : await register(field_values.Name, field_values.Email, field_values.Passwort);
          if (response === void 0) {
            await goto("/");
          } else {
      ...
      do {
        ...
        $$rendered = `<div class="${"outer svelte-1ewt7kf"}"><div class="${"main svelte-1ewt7kf"}"><div class="${"avatar svelte-1ewt7kf"}">${validate_component(UserAvatar, "UserAvatar").$$render($$result, { size: 32 }, {}, {})}</div>
		<h3>${page_mode === "login" ? `Anmelden` : `${page_mode === "register" ? `Registrieren` : ``}`}</h3>
		<form class="${"inputs svelte-1ewt7kf"}"${add_styles({
          "--distance": page_mode === "login" ? "25px" : "0px"
        })}>${each(typed_entries(required_fields[page_mode]), (required_field) => {
          let type = required_field[1].type, text = required_field[0];
          return `
				
				${validate_component(Input_field, "InputField").$$render(
            $$result,
            {
              text,
              ...
              value: field_values[text]
            },
            {
              value: ($$value) => {
                field_values[text] = $$value;
                $$settled = false;
              }
            },
            {}
			...
			${error_message ? `<p><mark>${escape2(error_message)}</mark></p>` : ``}
			<div class="${"btn svelte-1ewt7kf"}">${validate_component(Button, "Button").$$render(
          $$result,
          {
		...
		<p class="${"register svelte-1ewt7kf"}">${escape2(page_mode === "login" ? "Noch keinen Account?" : "Bereits ein Account?")}
      ...
      $$unsubscribe_current_auth_state();
      $$unsubscribe_page();
      return $$rendered;
    });
  }
...
var init__3 = __esm({
  ".svelte-kit/output/server/nodes/16.js"() {
    ...
    file3 = "_app/immutable/components/pages/login/_page.svelte-8a67cf72.js";
    imports3 = ["_app/immutable/components/pages/login/_page.svelte-8a67cf72.js", "_app/immutable/chunks/index-8132481b.js", "_app/immutable/chunks/navigation-93d63568.js", "_app/immutable/chunks/singletons-96e64e34.js", "_app/immutable/chunks/index-674a61bc.js", "_app/immutable/chunks/stores-88328872.js", "_app/immutable/chunks/button-e8e119c3.js", "_app/immutable/chunks/input_field-cec05477.js", "_app/immutable/chunks/auth_state-7a238df3.js", "_app/immutable/chunks/token_login-8de4a6c4.js", "_app/immutable/chunks/user_data-73545cb4.js", "_app/immutable/chunks/index-1d5742ea.js", "_app/immutable/chunks/datatypes-f51f27d5.js", "_app/immutable/chunks/loading_store-b28525d0.js", "_app/immutable/chunks/index-afe3ebdb.js"];
    stylesheets3 = ["_app/immutable/assets/_page-c552b2af.css", "_app/immutable/assets/button-9f2706db.css", "_app/immutable/assets/input_field-c61dede8.css"];
    fonts3 = [];
  }
});
...
// .svelte-kit/vercel-tmp/login/manifest.js
var manifest = {
  appDir: "_app",
  appPath: "_app",
...
// .svelte-kit/vercel-tmp/login/edge.js
